...
Point your terminal at the kubeconfig generated by `hetzner-k3s`:

```bash
export KUBECONFIG="$PWD/infra/hetzner-k3s/kubeconfig"
```

## 3. Apply the Traefik Addon Override
...
## 7. Install the Foundation Release

```bash
helmfile -f infra/helm/helmfile.yaml -l phase=foundation apply
```

## 8. Install Platform Postgres Secrets
...
kubectl apply -f infra/manifests/postgres/postgres-gitea-app-secret.yaml
```

## 9. Install Platform Postgres Instances
...
## 13. Deploy the Gitea Instance

```bash
helmfile -f infra/helm/helmfile.yaml -l phase=gitea apply
```

## 13.1. Run Services Bootstrap

This mints the onboarding Coder token, publishes the Coder template, stores the
Coder template ID, and mints the Gitea admin PAT used by onboarding.

```bash
helmfile -f infra/helm/helmfile.yaml -l phase=services-bootstrap apply
```

## 14. Apply OpenMercato CRM Secrets

```bash
kubectl apply -f infra/manifests/openmercato/openmercato-app-secrets.yaml
```

## 15. Deploy the OpenMercato CRM Instance
...
## 18. Generate `OPENMERCATO_API_KEY`
...
## 19. Configure the Onboarding App
